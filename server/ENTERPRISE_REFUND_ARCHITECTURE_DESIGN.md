...
**Status:** Architecture design + adversarial review complete. A follow-up cross-domain dependency sweep (Coupons, Wallet, Customer Credit, generic Approval-engine, Delivery-provider refund integration — none exist) found no further gaps. **The resulting architecture decision is now formally recorded in `ADR-001-SALES-PAYMENT-ARCHITECTURE.md`'s Phase 2 section (2026-07-20).** Implementation has not started and requires separate sign-off. No code, schema, model, service, controller, route, validation, or test has been created or modified in producing this document.
**Date:** 2026-07-20
