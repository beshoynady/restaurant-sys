...
- The shared engine is a fully self-contained plain-JS class — it owns the entire generic CRUD engine directly (create/getAll/findById/update/findOne/softDelete/restore/hardDelete/bulk*/exists/count/distinct/upsert/aggregate, transaction mechanics, plus the `beforeCreate`/`afterCreate`/etc. lifecycle hooks) rather than extending anything. It has no `.ts` dependency of any kind, which is what makes it safely importable from a plain `.js` file — a `.js` file importing a `.js`-specifier that only resolves to `.ts` source is an unreliable path under this project's `tsx` runtime (this was confirmed directly: it's what broke the server after the old `BaseRepository.ts` was deleted). The file itself was named `utils/BaseService.js` until **2026-07-12**, when it was renamed to **`utils/BaseRepository.js`** for naming clarity (it owns 100% of database access, so "Repository" is the accurate name — "Service" was a holdover from before the Repository/Service split below existed). Same class, same self-contained design, every importing module updated in the same pass — **not** a reintroduction of the deleted two-class `.ts` split described above.
- Every `<entity>.repository.js` in this rollout (pilot + Organization) extends `BaseRepository` directly — not a separate class. There is currently no TypeScript-only base class in this codebase; `BaseRepository.js` is the one and only shared CRUD engine, usable identically from JS or TS modules.
