...
`organization/brand`, `organization/brand-settings`, `organization/branch`, `organization/branch-settings`, `organization/delivery-area` — all five got the Repository Pattern split (`repository.*` + `service.* extends Repository`), same pattern as the pilot.

**Language note:** `Brand` and `BrandSettings` were briefly converted from JS to TS during this pass, then **reverted back to JS on explicit request** — the Repository Pattern split, all bug fixes, and `BrandSettings`' `BaseService`/`BaseController` alignment (it previously had neither) were kept; only the language changed back. `Brand`/`BrandSettings` are therefore `.js` + Repository Pattern, while `Branch`/`BranchSettings`/`DeliveryArea` remain `.ts` (they already were before this pass — untouched language-wise, only gained `repository.ts`). Project-wide JS→TS conversion stays deferred per `BACKEND_FOUNDATION.md` §5's existing policy (convert a module only when it comes up for rebuild) — this pass did not change that policy, it only reaffirmed it for Organization specifically.
