...
**Status:** §8/§20's `PreparationSettings` trimming recommendation was **implemented on 2026-07-21** — `PreparationTicketSettings`/`PreparationReturnSettings` were fully removed (not merely deprecated) and `PreparationSettings` was trimmed to `ticket`/`return`/`queue`/`sla` only, exactly as recommended here. §6-§19's Device Platform / Routing Platform design was **NOT implemented** — see `PLATFORM_DEVICE_ROUTING_ARCHITECTURE_VALIDATION.md`, which superseded this document's own naming/placement for that part before any code was written, and remains unimplemented, awaiting approval.
**Date:** 2026-07-20
