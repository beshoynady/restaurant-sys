...
**Status:** Proposed — **Phase 0 and Phase 1 approved and implemented (2026-07-19); Phase 1 refactored onto MongoDB transactions (2026-07-20). Phase 2 (Refund) implemented (2026-07-21)** — see the Phase 2 implementation record below. Phases 3-4 remain awaiting approval, not implemented.
**Date:** 2026-07-18 (Phase 0 landed 2026-07-19, Phase 1 landed 2026-07-19, Phase 1 transactional refactor landed 2026-07-20, Phase 2 architecture decided 2026-07-20)
**Supersedes (partially, see §0):** a factual correction to `ENTERPRISE_PAYMENT_ARCHITECTURE_DESIGN.md`'s Phase 0 recommendation. **Also supersedes this document's own original §9 Phase 2 sketch** (single-aggregate `SalesReturn.recordRefund()`, no `PreparationReturn` coordination, no settings-driven approval) — see the revised Phase 2 section below for why.
**Deciders:** owner approved Phase 0 and, separately, Phase 1 (both 2026-07-19); owner separately decided (2026-07-20) that MongoDB Transactions are the platform-wide standard mechanism for multi-document financial operations, and directed Phase 1's refactor onto it; owner directed the Phase 2 (Refund) architecture design, review, and cross-domain verification process that this section records the outcome of (2026-07-20). Phase 2 *implementation* still requires explicit sign-off, separate from this architecture decision. Phases 3-4 still require explicit sign-off before any further implementation begins.

**Platform architecture decision (2026-07-20): MongoDB Transactions are the standard mechanism for every multi-document financial write in this ERP.** Any operation that must atomically touch more than one collection as part of recording a financial fact (e.g. an Invoice balance + a Payment + one or more CashTransactions + a JournalEntry) runs inside a single MongoDB session/transaction via `BaseRepository#withTransaction()` — all writes commit together or none do. This decision was made explicitly to close two findings from the independent engineering audit of the original (non-transactional, best-effort) Phase 1 implementation: no session wrapped the invoice-balance-decrement → Payment-create sequence (a crash mid-sequence left an orphaned balance decrement with no Payment record), and a true concurrent double-submit with the same idempotency key could decrement the invoice balance twice before the unique-index guard caught the duplicate Payment document. This decision supersedes the original Phase 1 implementation's "best-effort, log-and-continue" posture for CashTransaction/JournalEntry creation — under the transactional model, a failure at any step now aborts and rolls back the whole payment rather than leaving a partially-recorded one.

**Phase 1 implementation record (2026-07-19, superseded in mechanics by the 2026-07-20 transactional refactor below; business behavior unchanged):** new `sales/payment` module (`payment.model.js/repository.js/service.js/controller.js/router.js/validation.js`), mounted at `POST/GET /sales/payments`, `GET /sales/payments/:id`. `PaymentService.recordPayment()` mirrors `PurchaseInvoice.recordPayment()`'s shape: atomically draws down the new `Invoice.balanceDue`/`amountPaid` fields via a MongoDB pipeline update (computes `status` — `OPEN`→`PARTIALLY_PAID`→`PAID` — from the post-decrement value in the same atomic op, avoiding the read-then-write race a plain `$inc` would have), creates one `CashTransaction` per tender (the module's first real production creator — closes `PAYMENT_LIFECYCLE_AUDIT.md`'s P4 finding), and posts a new `SALES_PAYMENT_RECEIPT` journal entry (Debit Cash/Card-Clearing, Credit Accounts Receivable — the settlement leg Phase 0 left pending). `CashTransaction.number` generation added to `cashier-shift-settings.service.js#getNextTransactionNumber` (a new `cashTransactionSequence` field, identical shape to the existing `shiftSequence`) — closes a gap discovered mid-implementation: `CashTransaction` had no number generator anywhere. New RBAC resource `"Payments"` added additively to `RESOURCE_ENUM`.

**Phase 1 transactional refactor — independent-audit remediation (2026-07-20):** an independent engineering audit of the above implementation confirmed five findings (#1-#5); #1-#3 (CashTransaction.cashRegister never propagated, split-tender GL collapse, unscoped cross-tenant CashRegister lookup) were fixed same-day in `payment.service.js` without touching architecture. Findings #4 (no transaction wrapping the multi-document write) and #5 (idempotency-key race under concurrent duplicate requests) were escalated to the owner as an Architecture Decision Proposal comparing MongoDB transactions, write-reordering, claim-first optimistic concurrency, and an outbox pattern — the owner decided MongoDB Transactions as the platform standard (see decision above) rather than any of the proposal's alternatives. Implementation: `PaymentService.recordPayment()` now runs its entire body — the idempotency re-check, the Invoice balance pipeline update, the Payment create, every tender's CashTransaction create, and the `SALES_PAYMENT_RECEIPT` JournalEntry post — inside one `this.withTransaction(session => ...)` call. `BaseRepository#withTransaction()` (used by 17+ existing modules) was changed to delegate to the MongoDB driver's own native `session.withTransaction()` instead of a hand-rolled start/commit/abort — the officially documented pattern, which retries a genuine `TransientTransactionError` (e.g. two concurrent transactions write-conflicting on the same Invoice document) within a time budget rather than a fixed attempt count; a fixed small retry count was tried first and empirically failed under this module's own multi-step transaction duration, which is what surfaced the need for the driver's time-budgeted version. `journalEntryService.createBalancedEntry()`/`postFromSource()` gained an optional externally-owned `session` parameter (backward compatible — every other existing caller is unaffected and keeps its self-contained transaction) so Payment's outer transaction can include the JournalEntry write rather than opening a second, unrelated one; `accountingSettingService.resolveForPosting()`/`getNextEntryNumber()`, `journalLineRepository.existsForSource()`, and `cashierShiftSettingsService.getNextTransactionNumber()` gained the same optional `session` parameter for the same reason. The prior "best-effort, log-and-swallow" behavior for CashTransaction/JournalEntry failures was removed — both now throw and roll back the whole attempt, per the platform decision above. Idempotency-under-concurrency: the idempotency-key check now runs a second time *inside* the transaction (session-scoped read) in addition to the pre-check outside it, so a retried transaction attempt (after a write-conflict) sees the winning attempt's already-committed Payment and returns it, instead of blindly re-attempting a balance decrement that would now fail as "insufficient balance." Full verification: `node --check` + `eslint` clean (0 errors, 0 warnings) on all 6 changed files; `sales-payment-recording.test.ts` expanded from 5 to 10 tests (added: concurrent duplicate payment with the same idempotencyKey, transaction rollback on CashTransaction validation failure, transaction rollback on journal-posting failure with all four operations — including an already-created CashTransaction — verified rolled back together) — all 10 pass; full regression sweep of the entire integration suite (75 suites / 327 tests, not just the payment-adjacent subset) — zero failures. No `tsc` used anywhere in this work, per standing instruction.

**Explicitly not done in Phase 1** (per §9's own scope): `Order.paymentStatus` removal (§7's recommendation, not assigned to any phase's roadmap item — a separate small cleanup task), Void/Refund logic (Phase 2), external gateway integration (Phase 4).

**Phase 2 architecture decision (2026-07-20): a two-aggregate, cross-bounded-context-reference Refund design, settings-driven approval, MongoDB transactions per aggregate.** Full design and its own adversarial review live in `server/ENTERPRISE_REFUND_ARCHITECTURE_DESIGN.md` (design pass) plus that same document's appended "Architecture Review" section (stress-test/verification pass) — this paragraph records the decision itself, not the reasoning; see that document for the full evidence trail. Summary: `SalesReturn` remains the Sales-context Aggregate Root for the financial/Refund concern (Step A `SALES_RETURN` revenue/tax/AR reversal, Step B `SALES_REFUND` cash settlement — mirrors the already-real `PurchaseReturn.approve()`/`recordRefund()` shape, reversed for AR, built on MongoDB transactions from the start rather than retrofitted). It does **not** absorb kitchen/inventory-disposition logic — that already has its own real, if dormant, Aggregate Root (`PreparationReturn`, Preparation bounded context, `items[].decision: WASTE/RETURN_TO_STOCK/RESELLABLE`, its own status lifecycle and approver chain), referenced by `SalesReturn` one-directionally by ID, exactly mirroring the live, unmodified `Order`/`PreparationTicket` precedent (`PreparationTicket.order` references `Order`; `Order` holds no back-reference — confirmed by source, not assumed). Approval is **not** RBAC-permission-based (the original design sketch's assumption) — it is driven by two already-existing, currently-dormant settings documents: `SalesReturnSettings` (`approvalThresholdAmount`, `decisionBy: [JobTitle]`, `maxReturnMinutes`, `refundTaxes`/`refundServiceCharge`/`refundDeliveryFee`/`generateAccountingEntry` toggles) for the financial approval, and `PreparationReturnSettings` (per-`preparationSection`, its own `decisionBy`/`requireSupervisorReview`/`maxReturnMinutesFromPreparation`) for the kitchen-disposition approval — two separate policies, not one unified threshold. A cross-domain dependency sweep (Coupons, Wallet, Customer Credit, generic Approval-engine, Delivery-provider refund integration) confirmed no further dormant refund-adjacent infrastructure exists beyond what the design document and its review already catalogued.

**Phase 2 implementation record (2026-07-21):** implemented under Enterprise Engineering Governance mode, re-verifying every load-bearing fact against current source before writing code (see `server/PREPARATION_SECTION_ENTERPRISE_WORK_CENTER_REVIEW.md`'s own governance precedent from earlier the same session). **One correction to this ADR's own Phase 2 decision paragraph above, found during that re-verification**: `PreparationReturnSettings` — named above as the kitchen-disposition approval settings source — **no longer exists**. It was fully deleted on 2026-07-21 (same day, earlier in this session) by the Preparation Settings Final Consolidation, folded into `PreparationSettings.return.*` (same `decisionBy: [JobTitle]` field, same convention, confirmed by that model's own header comment explicitly citing this ADR's Phase 2 decision). The architecture decision itself — two aggregates, settings-driven not RBAC-driven approval — is unchanged; only the settings model's name/location moved, before Phase 2 implementation ever started. Implementation used `PreparationSettings.return.decisionBy` throughout.

Scope delivered, matching the design review's corrected Blocker 1-3 findings exactly: (1) `warehouseDocumentService.postDocument()` and `wasteRecordService.approve()`/`.transition()` retrofitted with the `ownsSession` external-session pattern (Blocker 2's confirmed defect) — verified backward-compatible via the full existing `purchase-return`/`waste-record`/`inventory-count` suites, unchanged; (2) `SalesReturn` given real business logic (`requestRefund`/`approve`/`reject`/`settleRefund`), replacing the 16-line CRUD shell, plus `lockedUpdateFields` (closing the live security gap named in the design doc §15); (3) `PreparationReturn.finalize()` — new — wires `items[].decision` to real inventory movement for the first time (`RETURN_TO_STOCK`/`RESELLABLE` → `ReturnIssuance`; `WASTE` → restore-then-immediately-waste through the existing `WasteRecord{CustomerReturnWaste}` category, a net-zero physical stock change but a real, correctly-costed loss entry — the original sale already deducted the ingredients once, so there is nothing left to waste a second time without first conceptually restoring it); (4) `SalesReturn`'s service orchestrates `PreparationReturn` ticket creation post-commit (never inside its own transaction), grouped by `preparationSection` exactly as `order.service.js#createTicketsFromOrder` already groups by section for `Order`; (5) `order.service.js#cancelItem` gained the paid-order guard the design review's own stress test identified as a real, previously-undocumented bypass; (6) `SALES_REFUND` added to `JournalLine.sourceType`; (7) `Invoice.items[].refundedQuantity` added for line-level double-refund prevention; (8) `salesReturnSettingsService` gained `resolveForBranch()`/`getNextReturnSerial()` (previously pure CRUD, unreadable by name).

**Two additional, genuine defects found and fixed while building this phase** (not scope creep — both were live correctness bugs in already-shipped Phase 1 code, caught because Phase 2 needed the identical idempotency mechanism and hit them for real): (a) `Payment`'s and (now) `SalesReturn`'s idempotency-key uniqueness index used `sparse: true` on a **compound** index — MongoDB's sparse flag only excludes a document from a compound index when *every* indexed field is absent; since `brand`/`invoice` (or `brand`/`originalInvoice`) are always present, a document with no idempotency key was never actually excluded, so two no-key requests against the same invoice collided. Fixed on both models by switching to `partialFilterExpression: { idempotencyKey: { $exists: true } } }` — the documented, correct mechanism for this exact case — plus removing each model's `default: null` (which produced the always-present value the sparse flag couldn't skip in the first place). (b) `warehouseDocumentService.postDocument()`'s Phase-2 session retrofit initially threaded the external session into every write but not its own opening `findOne` read, making a document created earlier in the *same* external transaction invisible to it (transaction-isolation semantics: an out-of-session read cannot see the transaction's own uncommitted writes) — fixed by adding `.session(externalSession)` to that read.

Verified: `node --check` + `eslint` clean (0 errors) on every changed/new file. New suites: `sales-return-refund-lifecycle.test.ts` (5 tests — auto-approved Step A+B, over-refund rejection, idempotency replay, decisionBy authorization block/allow, reject-releases-claim + deferred settlement), `preparation-return-inventory-posting.test.ts` (4 tests — status-guard, RETURN_TO_STOCK, WASTE, re-finalize rejection), plus a new `cancelItem` paid-order-guard test. Full platform regression: **80 suites / 354 tests, 0 failures** (up from the pre-phase 78/344 baseline — exactly the expected +2 suites/+10 tests, zero collateral damage), including every other `postDocument()`/`wasteRecordService.approve()` caller (`purchase-return-engine`, `waste-record-engine`, `inventory-count-engine`) and the existing `sales-payment-recording` suite (unaffected by the idempotency-index fix).

**Still correctly unimplemented, per this phase's own explicit scope**: `PreparationSettings.return.decisionBy`'s *kitchen* gate is enforced by `finalize()` when configured but fails open when empty (matches this platform's established convention for unconfigured policy gates elsewhere — not a gap introduced here); Void (distinct from Refund, §2.2, out of scope); Wallet/Gift Card/Store Credit settlement (no real ledger exists anywhere, §2.1, not invented); fraud-pattern-detection reporting (§20 of the design doc, named as a should-follow-soon item, not blocking); Phases 3-4 (cash/shift reconciliation verification, external payment providers) remain unapproved.
...
- **Risk:** medium — new concurrency-sensitive balance decrement; originally mitigated by the platform's proven atomic-`findOneAndUpdate` pattern alone, now additionally wrapped in a full MongoDB transaction (2026-07-20 refactor, see implementation record near the top of this document) per the platform's standing MongoDB Transactions decision — closes the two gaps ("no session across the multi-document write" and "idempotency-key race under true concurrency") an independent audit found in the original implementation.
- **Acceptance criteria:** ✅ a payment recorded against an invoice correctly decrements `balanceDue`, ✅ creates one correctly-`cashRegister`-linked `CashTransaction` per tender, ✅ posts one balanced `JournalEntry` (one debit line per distinct resolved cash/card account for split tenders, not collapsed to one), ✅ is idempotent under retry and under true concurrency (P10/G7 closed via a sparse unique index plus an in-transaction re-check, not deferred), ✅ `CashierShift.countShift()` can now reconcile against real data (verified via the existing `cashier-shift-reconciliation-engine` suite, unaffected/still passing), ✅ any step's failure (insufficient balance, invalid tender, no postable GL account) rolls back every write already made in that attempt — verified by dedicated rollback tests.
- **Rollback:** additive collection; disable the new endpoints to revert to Phase-0-only behavior.
- **Testing strategy:** new `sales-payment-recording.test.ts` — 5 tests (happy path, partial payment, over-balance rejection, idempotent replay, concurrent-payment race) mirroring `purchase-invoice`'s payment-recording test structure. All pass; full regression sweep (9 suites/37 tests) zero failures.

### Phase 2 — Refund Aggregate — **ARCHITECTURE DECIDED (2026-07-20); implementation not started, requires separate sign-off**
- **Scope (revised, supersedes the original single-aggregate sketch above):** two aggregates, not one.
  - `SalesReturn` (Sales context, real service replacing today's 16-line CRUD shell): Step A posts `SALES_RETURN` (revenue/tax/discount/serviceCharge/deliveryFee contra, credit AR — all five control accounts already exist on `AccountingSettings`, unused today), conditional on `SalesReturnSettings.refundTaxes`/`refundServiceCharge`/`refundDeliveryFee`/`generateAccountingEntry`. Step B posts `SALES_REFUND` (new `JournalLine.sourceType` value — the one required schema addition) + `CashTransaction{REFUND, OUTFLOW}` per resolved account (split-tender-safe, reusing Payment's own per-tender grouping fix). Step A and B are the same transaction when settled together, two separate transactions otherwise — mirrors `PurchaseReturn.approve()`/`recordRefund()` being two calls today, each now individually wrapped in `withTransaction`.
  - `PreparationReturn` (Preparation context, already has a real status guard, needs its `decision` field wired to real inventory posting): its own finalization triggers `ReturnIssuance` (restored) or `WasteRecord{CustomerReturnWaste}` (wasted) via the now-session-safe `warehouseDocumentService.postDocument()`/`wasteRecordService.approve()`, in its **own** transaction, referenced from `SalesReturn` by ID only (`PreparationReturn.returnInvoice`) — not embedded, not the same transaction.
- **Dependencies:** Phase 1 (a refund needs a real payment to refund against); **and, newly identified as a hard prerequisite, not optional polish:** `warehouseDocumentService.postDocument()` and `wasteRecordService.approve()` must first receive the `ownsSession`-guard retrofit (§13.9 of the design review) — without it, `PreparationReturn`'s inventory posting cannot honestly participate in the MongoDB-transactions standard at all.
- **Risk:** medium-high — genuinely more coordination than Phase 1 (two aggregates, not one), though the *pattern* (cross-context reference, service orchestrates child-ticket creation) is proven and unmodified from the live `Order`/`PreparationTicket` precedent, not novel.
- **Acceptance criteria:** ✅ `SalesReturn`'s Step A/B match Phase 1's transactional/idempotent/split-tender-correct shape, mirrored for the outflow direction; ✅ `PreparationReturn.decision` actually drives real inventory movement (today it drives nothing); ✅ approval is enforced via `SalesReturnSettings.decisionBy`/`approvalThresholdAmount` (financial) and `PreparationReturnSettings.decisionBy` (kitchen), each with a real `resolveForBranch`-style settings-resolution method (neither settings service has one today); ✅ `order.service.js#cancelItem` gains a guard rejecting cancellation of an item whose parent `Invoice.amountPaid > 0` (a newly-identified bypass — today a paid order can still be cancelled through `cancelItem`, skipping Refund's accounting entirely); ✅ `sales-return` gains `lockedUpdateFields` (a live, pre-existing security gap in the aggregate this phase builds on, not new-phase scope creep).
- **Rollback:** additive; disable endpoints. `PreparationReturn` already exists and is currently inert — wiring its `decision` field to real postings is additive to that collection too, no migration.
- **Testing strategy:** mirror `purchase-return`'s existing `recordRefund` test suite structure for `SalesReturn`'s Step A/B; new coverage for `PreparationReturn`'s finalization → inventory-posting path (restored vs. wasted vs. ignored); a dedicated test for the `cancelItem` paid-order guard; a dedicated test proving `warehouseDocumentService.postDocument()`'s session retrofit doesn't change behavior for any of its existing non-Refund callers (`PurchaseReturn`, `InventoryCount`, `WasteRecord`'s other categories) — same backward-compatibility verification style Phase 1 used for `journalEntryService`'s own retrofit.
...
**Status: Phase 0, Phase 1, and Phase 1's transactional refactor are approved and implemented (see the implementation records near the top of this document). Phases 2-4 remain decision records awaiting approval — no code, schema, or migration for those phases has been created or modified.**
