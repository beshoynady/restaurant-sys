...
**Status:** Proposed — **Phase 0 and Phase 1 approved and implemented (2026-07-19).** Phases 2-4 remain awaiting approval, not implemented.
**Date:** 2026-07-18 (Phase 0 landed 2026-07-19, Phase 1 landed 2026-07-19)
**Supersedes (partially, see §0):** a factual correction to `ENTERPRISE_PAYMENT_ARCHITECTURE_DESIGN.md`'s Phase 0 recommendation.
**Deciders:** owner approved Phase 0 and, separately, Phase 1 (both 2026-07-19). Phases 2-4 still require explicit sign-off before any further implementation begins.

**Phase 1 implementation record:** new `sales/payment` module (`payment.model.js/repository.js/service.js/controller.js/router.js/validation.js`), mounted at `POST/GET /sales/payments`, `GET /sales/payments/:id`. `PaymentService.recordPayment()` mirrors `PurchaseInvoice.recordPayment()`'s shape: atomically draws down the new `Invoice.balanceDue`/`amountPaid` fields via a MongoDB pipeline update (computes `status` — `OPEN`→`PARTIALLY_PAID`→`PAID` — from the post-decrement value in the same atomic op, avoiding the read-then-write race a plain `$inc` would have), creates one `CashTransaction` per tender (the module's first real production creator — closes `PAYMENT_LIFECYCLE_AUDIT.md`'s P4 finding), and posts a new `SALES_PAYMENT_RECEIPT` journal entry (Debit Cash/Card-Clearing, Credit Accounts Receivable — the settlement leg Phase 0 left pending). Idempotency (§18's G7) closed via a sparse `{brand, invoice, idempotencyKey}` unique index, the same "compound index as concurrency guard" idiom already used everywhere in this codebase, not a new header-based mechanism. `CashTransaction.number` generation added to `cashier-shift-settings.service.js#getNextTransactionNumber` (a new `cashTransactionSequence` field, identical shape to the existing `shiftSequence`) — closes a gap discovered mid-implementation: `CashTransaction` had no number generator anywhere. New RBAC resource `"Payments"` added additively to `RESOURCE_ENUM`. Full verification: `node --check` + `eslint` clean (0 errors) on all 13 new/changed files, 5 new integration tests (happy-path full payment, partial payment, over-balance rejection, idempotent replay, concurrent-payment race) all pass, full regression sweep (9 suites/37 tests spanning invoice, cashier-shift, financial-statements, finance-reports, executive-dashboard) zero failures. No `tsc` used anywhere in this phase, per standing instruction.
...
### Phase 1 — Payment Aggregate — **DONE (2026-07-19)**
...
- **Risk:** medium — new concurrency-sensitive balance decrement; mitigated by reusing the platform's proven atomic-`findOneAndUpdate` pattern (a MongoDB pipeline update, computing `status` from the post-decrement value in the same atomic op — a small, justified evolution of the plain-`$inc` idiom, not a novel mechanism).
- **Acceptance criteria:** ✅ a payment recorded against an invoice correctly decrements `balanceDue`, ✅ creates exactly one `CashTransaction` per tender, ✅ posts one balanced `JournalEntry`, ✅ is idempotent under retry (P10/G7 closed via a sparse unique index, not deferred), ✅ `CashierShift.countShift()` can now reconcile against real data (verified via the existing `cashier-shift-reconciliation-engine` suite, unaffected/still passing).
